...
async def on_message(message):
    ...
    while(not terminate):
      ...
      while(not worked):
        ...
        try: 
          ...
          os.makedirs('./images/1{}'.format(name))
          r = requests.get(link, stream=True)
          r.raw.decode_content = True
          filepath = './images/1{}'.format(name) + '/' + name + '.jpeg'
      ...
      try:
          ...
          shutil.rmtree('./images/1{}'.format(name))
      except:
  ...
  async def cropped_game(message, use_artist, easy, query):
    ...
    while(not terminate):
      worked = False
      while(not worked and not terminate):
        try: 
          artist, title, date, link = choose_painting3(easy, query)
          while(not is_valid(link)):
            artist, title, date, link = choose_painting3(easy, query)
          ...
          r = requests.get(link, stream=True)
          r.raw.decode_content = True
          filepath = './images/2{}'.format(name) + '/' + name + '.jpeg'
          with open(filepath, 'wb') as f:
            shutil.copyfileobj(r.raw, f)
        ...
        while(not going):
          try:
            ...
            if(msg.content.startswith('.end')):
              ...
              terminate = True
              going = True
              shutil.rmtree('./images/2{}'.format(name))
              active_channels.remove(message.channel.id)
            elif(msg.content.startswith('.score')):
              game_over_embed = discord.Embed(title="Score")
              game_over_embed.add_field(name="1st clue:", value=user_scores[0])
    ...
    while(j < num_questions):
      ...
      while(not worked):
        try:
          ...
          r = requests.get(link, stream=True)
          r.raw.decode_content = True
          filepath = './images/3{}'.format(name) + '/' + name + '.jpeg'
          with open(filepath, 'wb') as f:
            shutil.copyfileobj(r.raw, f)
          worked = True
          if(use_artist):
        ...
        if(buzzed):
          ...
          try:
            ...
            if(correct):
              ...
              shutil.rmtree('./images/3{}'.format(name))
              buzzes = []
            else:
              change = False
              if(i != 5):
  ...
  if message.content.startswith('.start') and message.channel.id not in active_channels:
    ...
    if(valid):
      ...
      if(crop):
        print("{} is starting cropped, Easy={}".format(message.author.name, str(easy)))
        await cropped_game(message, use_artist, easy, query)
        print("{} is ending cropped".format(message.author.name))
      else:
        print("{} is starting uncropped, Easy={}".format(message.author.name, str(easy)))
        await uncropped_game(message, use_artist, easy, query)
        print("{} is ending uncropped".format(message.author.name))
  ...
  if message.content.startswith('.multiplayer') and message.channel.id not in active_channels:
    ...
    if(valid):
      print("{} is starting multiplayer, Easy={}".format(message.author.name, str(easy)))
      await multiplayer_game(message, use_artist, easy, num_questions)
      print("{} is ending multiplayer".format(message.author.name))
